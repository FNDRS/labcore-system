...
import { getUserGroups, GROUP_TO_ROUTE, type AuthGroup } from "@/lib/auth";
...
function AuthProviderInner({ children }: { children: ReactNode }) {
  ...
  const redirectToRoleRoute = useCallback(async () => {
    ...
    try {
      const groups = groupsRef.current.length > 0 ? groupsRef.current : state.groups;
...
// --- Guard (redirects unauthenticated users to login only; RBAC is server-side in layout) ---
