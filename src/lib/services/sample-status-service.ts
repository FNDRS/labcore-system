 ...
 */
export async function markSampleReceived(sampleId: string, userId: string): Promise<StatusResult> {
  ...
  await emitAudit(AUDIT_ENTITY_TYPES.SAMPLE, sampleId, AUDIT_ACTIONS.SPECIMEN_RECEIVED, userId);
  return { ok: true };
}
...
export async function markSampleInProgress(
  ...
  await emitAudit(AUDIT_ENTITY_TYPES.SAMPLE, sampleId, AUDIT_ACTIONS.SPECIMEN_IN_PROGRESS, userId);
  return { ok: true };
}
 ...
 */
export async function markSampleCompleted(sampleId: string, userId: string): Promise<StatusResult> {
  const { data: sample } = await cookieBasedClient.models.Sample.get({
    id: sampleId,
  });
  if (!sample?.id) return { ok: false, error: "Muestra no encontrada" };
 ...
 */
export async function reprintSampleLabel(sampleId: string, userId: string): Promise<StatusResult> {
  const { data: sample } = await cookieBasedClient.models.Sample.get({
    id: sampleId,
  });
  if (!sample?.id) return { ok: false, error: "Muestra no encontrada" };

  await emitAudit(AUDIT_ENTITY_TYPES.SAMPLE, sampleId, AUDIT_ACTIONS.LABEL_REPRINTED, userId);
 ...
 */
export async function markSampleRejected(sampleId: string, userId: string): Promise<StatusResult> {
  const { data: sample } = await cookieBasedClient.models.Sample.get({
    id: sampleId,
  });
  if (!sample?.id) return { ok: false, error: "Muestra no encontrada" };
  ...
  await emitAudit(AUDIT_ENTITY_TYPES.SAMPLE, sampleId, AUDIT_ACTIONS.SPECIMEN_REJECTED, userId);
  return { ok: true };
}
