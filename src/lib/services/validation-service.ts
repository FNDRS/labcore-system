...
function hasConflict(currentUpdatedAt: string | null, expectedUpdatedAt?: string | null): boolean {
...
async function syncSampleStatusWhenAllExamsTerminal(
  ...
  if (sampleUpdate.errors?.length || !sampleUpdate.data?.id) {
    throw new Error(
      mutationErrorMessage(sampleUpdate.errors, "No se pudo actualizar el estado de la muestra")
    );
  }
...
export async function approveExam(
  ...
  if (updateResult.errors?.length || !updateResult.data?.id) {
    return {
      ok: false,
      error: mutationErrorMessage(updateResult.errors, "No se pudo aprobar el examen"),
    };
  }

  await emitAudit(AUDIT_ENTITY_TYPES.EXAM, examId, AUDIT_ACTIONS.EXAM_APPROVED, userId, {
    sampleId: exam.sampleId,
    comments: comments?.trim() || undefined,
  });
  await syncSampleStatusWhenAllExamsTerminal(exam.sampleId, userId);

  const updatedAt = (updateResult.data as { updatedAt?: string | null } | null)?.updatedAt ?? null;
...
export async function rejectExam(
  ...
  if (updateResult.errors?.length || !updateResult.data?.id) {
    return {
      ok: false,
      error: mutationErrorMessage(updateResult.errors, "No se pudo rechazar el examen"),
    };
  }

  await emitAudit(AUDIT_ENTITY_TYPES.EXAM, examId, AUDIT_ACTIONS.EXAM_REJECTED, userId, {
    sampleId: exam.sampleId,
    reason: reason.trim(),
    comments: comments?.trim() || undefined,
  });
...
export async function createIncidence(
  ...
  if (shouldMoveExamToReview(type) && exam.status === "ready_for_validation") {
    ...
    if (reviewUpdate.errors?.length || !reviewUpdate.data?.id) {
      return {
        ok: false,
        error: mutationErrorMessage(reviewUpdate.errors, "No se pudo mover el examen a revisión"),
      };
    }
  }

  await emitAudit(AUDIT_ENTITY_TYPES.EXAM, examId, AUDIT_ACTIONS.INCIDENCE_CREATED, userId, {
    sampleId: exam.sampleId,
    type: type.trim(),
