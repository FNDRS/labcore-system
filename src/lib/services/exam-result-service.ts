...
function serializeResultsForAwsJson(results: ResultsRecord): string | null {
  try {
...
function emitAudit(
  ...
  const serializedMetadata = metadata == null ? undefined : JSON.stringify(metadata);
  return cookieBasedClient.models.AuditEvent.create({
    ...
    if (result.errors?.length || !result.data?.id) {
      throw new Error(
        mutationErrorMessage(result.errors, "No se pudo registrar el evento de auditoría")
      );
    }
  });
}
 ...
 * Emits EXAM_STARTED.
 */
export async function markExamStarted(examId: string, userId: string): Promise<ExamResultStatus> {
  ...
  if (startUpdate.errors?.length || !startUpdate.data?.id) {
    return {
      ok: false,
      error: mutationErrorMessage(startUpdate.errors, "No se pudo iniciar el examen"),
    };
  }
  await emitAudit(AUDIT_ENTITY_TYPES.EXAM, examId, AUDIT_ACTIONS.EXAM_STARTED, userId, {
...
export async function saveExamDraft(
  ...
  if (draftUpdate.errors?.length || !draftUpdate.data?.id) {
    return {
      ok: false,
      error: mutationErrorMessage(draftUpdate.errors, "No se pudo guardar el borrador"),
    };
  }
  await emitAudit(AUDIT_ENTITY_TYPES.EXAM, examId, AUDIT_ACTIONS.EXAM_RESULTS_SAVED, userId, {
    sampleId: exam.sampleId,
    draft: true,
  });
...
export async function finalizeExam(
  ...
  if (finalizeUpdate.errors?.length || !finalizeUpdate.data?.id) {
    return {
      ok: false,
      error: mutationErrorMessage(finalizeUpdate.errors, "No se pudo finalizar el examen"),
    };
  }
  await emitAudit(AUDIT_ENTITY_TYPES.EXAM, examId, AUDIT_ACTIONS.EXAM_RESULTS_SAVED, userId, {
    sampleId: exam.sampleId,
 ...
 */
export async function sendToValidation(examId: string, userId: string): Promise<ExamResultStatus> {
  const { data: exam } = await cookieBasedClient.models.Exam.get({
    id: examId,
  });
  if (!exam?.id) return { ok: false, error: "Examen no encontrado" };
  ...
  if (sendUpdate.errors?.length || !sendUpdate.data?.id) {
    return {
      ok: false,
      error: mutationErrorMessage(sendUpdate.errors, "No se pudo enviar a validación"),
    };
  }
  await emitAudit(AUDIT_ENTITY_TYPES.EXAM, examId, AUDIT_ACTIONS.EXAM_SENT_TO_VALIDATION, userId, {
    sampleId: exam.sampleId,
  });
...
async function syncSampleStatusWhenExamSentToValidation(
  ...
  const allReady = exams.every(
    (e) => e.status === "ready_for_validation" || e.status === "approved" || e.status === "rejected"
  );
  ...
  await emitAudit(AUDIT_ENTITY_TYPES.SAMPLE, sampleId, AUDIT_ACTIONS.SPECIMEN_COMPLETED, userId, {
