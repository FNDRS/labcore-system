...
import { getDashboardStats, listPendingValidation } from "@/lib/repositories/supervisor-repository";
import {
  approveExam,
  createIncidence,
  rejectExam,
...
import type { SupervisorDashboardStats, ValidationQueueItem } from "@/lib/types/validation-types";
...
export const fetchSupervisorDashboard = cache(async (): Promise<SupervisorDashboardData> => {
  await requireSupervisorAuth();
  const [stats, pending] = await Promise.all([getDashboardStats(), listPendingValidation()]);
