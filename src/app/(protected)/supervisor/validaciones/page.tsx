...
function parseFilters(searchParams: {
  ...
  const statusFilter = (
    searchParams.status === "all" ? "all" : "pending"
  ) as ValidationQueueStatusFilter;

  const flag = searchParams.flag?.trim();
  const validFlags: ValidationQueueFilterFlag[] = ["critico", "atencion", "normal", "abnormal"];
  const flagFilter =
    flag && validFlags.includes(flag as ValidationQueueFilterFlag)
...
export default async function SupervisorValidacionesPage({ searchParams }: PageProps) {
  const params = await searchParams;
  const filters = parseFilters(params);
  const initialFeedback =
    params.feedback === "approved" || params.feedback === "rejected" ? params.feedback : null;
