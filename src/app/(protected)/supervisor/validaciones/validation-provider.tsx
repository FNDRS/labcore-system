...
const ValidationProviderContext = createContext<ValidationProviderContextValue | null>(null);

export function useValidationProvider(): ValidationProviderContextValue {
  const ctx = useContext(ValidationProviderContext);
  if (!ctx) {
    throw new Error("useValidationProvider must be used within ValidationProvider");
  }
  return ctx;
}

function matchesSearchQuery(item: ValidationQueueItem, query: string): boolean {
  if (!query.trim()) return true;
  const q = query.trim().toLowerCase();
  const patientMatch = item.patientName.toLowerCase().includes(q);
  const accessionMatch = item.accessionNumber?.toLowerCase().includes(q) ?? false;
...
export function ValidationProvider({
  ...
  const filters: ValidationFilterState = useMemo(
    () => ({
      ...
      fromDate: initialFilters.fromResultedAt ? initialFilters.fromResultedAt.slice(0, 10) : "",
      toDate: initialFilters.toResultedAt ? initialFilters.toResultedAt.slice(0, 10) : "",
      searchQuery,
    }),
    [
  ...
  const visibleItems = useMemo(
    () => (reviewedId ? initialItems.filter((item) => item.examId !== reviewedId) : initialItems),
    [initialItems, reviewedId]
  );

  const filteredItems = useMemo(() => {
    return visibleItems.filter((item) => matchesSearchQuery(item, searchQuery));
  }, [visibleItems, searchQuery]);

  const pendingCount = useMemo(
    () => visibleItems.filter((item) => item.status === "ready_for_validation").length,
    [visibleItems]
  );
  ...
  const criticalCount = useMemo(
    () =>
      visibleItems.filter(
        (item) => item.clinicalFlag !== "normal" || item.hasReferenceRangeViolation
      ).length,
    [visibleItems]
  );
  ...
  const buildSearchParams = useCallback(
    (overrides: Partial<ValidationFilterState>) => {
      const status = overrides.statusFilter ?? filters.statusFilter;
