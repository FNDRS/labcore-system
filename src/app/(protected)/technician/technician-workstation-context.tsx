...
const TechnicianWorkstationContext =
	createContext<TechnicianWorkstationContextValue | null>(null);
...
export function useTechnicianWorkstation(): TechnicianWorkstationContextValue {
	...
	if (!ctx) {
		throw new Error(
			"useTechnicianWorkstation must be used within TechnicianWorkstationProvider",
		);
	}
	return ctx;
}
...
export function TechnicianWorkstationProvider({
	...
	const [selectedDetail, setSelectedDetail] =
		useState<SampleWorkstationDetail | null>(null);
	...
	const loadSamples = useCallback(async () => {
		...
		try {
			...
			setSamplesError(
				err instanceof Error ? err.message : "Error al cargar muestras",
			);
			setSamples([]);
		} finally {
	...
	const selectedSample =
		selectedDetail ??
		(selectedId ? samples.find((s) => s.id === selectedId) ?? null : null);
