...
import { fetchPendingWorkOrders, fetchPendingSamples } from "./actions";
...
async function WorkOrdersSection() {
  ...
  return (
    <Card>
      ...
      <CardContent>
        {workOrders.length === 0 ? (
          <p className="text-muted-foreground text-sm">No hay órdenes pendientes.</p>
        ) : (
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Muestras pendientes y recibidas listas para procesar</CardDescription>
      </CardHeader>
      <CardContent>
        {samples.length === 0 ? (
          <p className="text-muted-foreground text-sm">No hay muestras en cola.</p>
        ) : (
...
export default function TechnicianPage() {
  return (
    <div className="container mx-auto max-w-6xl space-y-8 py-8 px-4">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Panel Técnico</h1>
        <p className="text-muted-foreground mt-1">Dashboard y recepción de muestras</p>
      </div>
