...
export async function markExamStartedAction(examId: string): Promise<ExamResultStatus> {
  await runWithAmplifyServerContext({
    nextServerContext: { cookies },
    operation: (ctx) => requireAuthWithGroup(ctx, "tecnico"),
  });
...
export async function sendToValidationAction(examId: string): Promise<ExamResultStatus> {
  await runWithAmplifyServerContext({
    nextServerContext: { cookies },
    operation: (ctx) => requireAuthWithGroup(ctx, "tecnico"),
  });
