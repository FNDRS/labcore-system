...
export function filterRows(rows: SampleWorkstationRow[], filter: Filter): SampleWorkstationRow[] {
  if (filter === "All") return rows;
  if (filter === "Processing")
    return rows.filter((r) => r.status === "Processing" || r.status === "Waiting Equipment");
  if (filter === "Received")
    return rows.filter((r) => r.status === "Awaiting Receipt" || r.status === "Received");
