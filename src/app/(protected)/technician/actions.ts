...
export async function lookupSampleByBarcodeAction(
  ...
  if (MOCK_TECHNICIAN) {
    const mock = MOCK_MUESTRAS.find(
      (s) => s.sampleId.toLowerCase().includes(code.trim().toLowerCase()) || s.id === code.trim()
    );
...
export const fetchOperativeDashboard = cache(async () => {
  if (MOCK_TECHNICIAN) {
    ...
    const urgentCount = queue.filter(
      (r) => r.priority === "Urgent" && r.status !== "Complete"
    ).length;
    const lastScanned = queue[0] ? { sampleId: queue[0].sampleId, status: queue[0].status } : null;
    return {
      nextSample,
      urgentCount,
      queueRows: queue,
